...
Dedupe::DB::File - Dedupe::DB class for the file table
...
=head2 base
  ...
  # Returns 'Dedupe::DB'
  my $namespace = Dedupe::DB::File->base;
...
=head2 load
  ...
  my $object = Dedupe::DB::File->load( $id );
...
It returns a C<Dedupe::DB::File> object, or throws an exception if the
object does not exist.

=head2 select

  # Get all objects in list context
  my @list = Dedupe::DB::File->select;
  
  # Get a subset of objects in scalar context
  my $array_ref = Dedupe::DB::File->select(
...
C<FROM file> section of the query, followed by variables
to be bound to the placeholders in the SQL phrase. Any SQL that is
compatible with SQLite can be used in the parameter.

Returns a list of B<Dedupe::DB::File> objects when called in list context, or a
reference to an C<ARRAY> of B<Dedupe::DB::File> objects when called in scalar
context.
...
following, except with an O(1) memory cost instead of O(n).
  ...
  foreach ( Dedupe::DB::File->select ) {
      print $_->id . "\n";
  }

You can filter the list via SQL in the same way you can with C<select>.

  Dedupe::DB::File->iterate(
      'order by ?', 'id',
      sub {
          print $_->id . "\n";
      }
...
references when used in this form.
  ...
  Dedupe::DB->iterate(
      'select name from file order by id',
      sub {
          print $_->[0] . "\n";
      }
  );
...
=head2 count
  ...
  my $rows = Dedupe::DB::File->count;
  
  # How many objects 
  my $small = Dedupe::DB::File->count(
      'where id > ?',
      1000,
  );
...
Returns a new L<Dedupe::DB::File> object.

=head2 create

  my $object = Dedupe::DB::File->create(
      id => 'value',
      action => 'value',
      mtime => 'value',
      size => 'value',
...
L<Dedupe::DB::File> object, inserts the appropriate row into the
L<file> table, and then returns the object.
...
Returns a new L<file> object, or throws an exception on
...
=head2 delete
  ...
  # Delete multiple rows from the file table
  Dedupe::DB::File->delete('where id > ?', 1000);

The C<delete> method can be used in a class form and an instance form.

When used on an existing B<Dedupe::DB::File> instance, the C<delete> method
removes that specific instance from the C<file>, leaving
...
phrase to be added after the C<DELETE FROM file> section
...
=head2 truncate
  ...
  # Delete all records in the file table
  Dedupe::DB::File->truncate;
...
The file table was originally created with the
...
Dedupe::DB::File is part of the L<Dedupe::DB> API.
